$(() => {
  ...
  let missedBalloons = 1;
  let slowBallons = $('.slow-area')[Math.floor(Math.random()*$('.slow-area').length)];
  let mediumBallons = $('.medium-area')[Math.floor(Math.random()*$('.slow-area').length)];
  let fastBallons = $('.fast-area')[Math.floor(Math.random()*$('.slow-area').length)];
  const pop = $('li');
